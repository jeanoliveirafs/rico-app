...
`),n=new Blob([`﻿`+t],{type:`text/csv;charset=utf-8`}),i=URL.createObjectURL(n),a=document.createElement(`a`);a.href=i,a.download=`rico-${D()}.csv`,a.click(),URL.revokeObjectURL(i)}function ie(){let e=s.transactions.slice();c.txFilterMonth&&(e=e.filter(e=>e.date?.startsWith(c.txFilterMonth))),c.txFilterCat&&(e=e.filter(e=>e.category===c.txFilterCat)),c.txFilterType&&(e=e.filter(e=>e.type===c.txFilterType));let t=e.filter(e=>e.type===`income`).reduce((e,t)=>e+t.amount,0),n=e.filter(e=>e.type===`expense`).reduce((e,t)=>e+t.amount,0),i=c.txFilterMonth?` — ${c.txFilterMonth}`:``,a=new Date().toLocaleDateString(`pt-BR`),o=e.map(e=>{let t=O(r,e.category),n=e.type===`income`?`#16a34a`:`#dc2626`,i=e.type===`income`?`+`:`-`;return`<tr><td>${T(e.date)}</td><td>${e.type===`income`?`Entrada`:`Saída`}</td><td>${k(e.desc)}</td><td>${t.emoji} ${t.label}</td><td style="text-align:right;color:${n};font-weight:600">${i}${w(e.amount)}</td></tr>`}).join(``),l=`<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>RICO — Relatório${i}</title>
  ...
  <div style="margin-bottom:16px; padding: 4px 2px">
    ...
    <p style="font-size:12px; color:var(--muted2); margin-top:2px; font-weight:500">${new Date().toLocaleDateString(`pt-BR`,{weekday:`short`,month:`short`,day:`numeric`})} • Desenvolvido por Jean Oliveira</p>
  </div>`,y=`
  <div class="card" style="padding:20px; background:var(--bg2)">
    <div class="rb">
      <span class="sl" style="font-size:11px; font-weight:800; letter-spacing:0.08em; color:var(--muted2)">SALDO DISPONÍVEL</span>
    ...
    <div class="rw" style="gap:8px">
      <span class="bg2" style="background:var(--green-bg); color:var(--green); font-size:11px; font-weight:700">📈 Fluxo do mês: ${w(m-h)}</span>
    </div>
  </div>`,b=`
  <div class="card" style="padding:18px">
    <div class="rb" style="margin-bottom:8px">
      <span style="font-size:16px; font-weight:800; color:var(--text)">Meta Financeira (R$ 100k)</span>
  ...
  <!-- CARD PRINCIPAL DE GANHOS DO PERÍODO -->
  ...
  <!-- CARDS MAIORES GASTOS NO PERÍODO -->
  <div class="g2">
    <div class="sc" style="border-left:4px solid var(--red)">
      <div class="rw" style="gap:6px"><span style="font-size:14px">${_.emoji}</span> <span style="font-size:12px; font-weight:700; color:var(--text)">${_.label}</span></div>
      <span class="mo" style="font-size:18px; font-weight:800; color:var(--text); margin-top:4px">${w(_.amount)}</span>
      <div class="pm" style="margin-top:6px; background:var(--bg3)"><div class="pf" style="width:${_.pct}%; background:var(--red)"></div></div>
      <span class="ti" style="font-size:10px; color:var(--muted2)">${_.pct}% das despesas</span>
    </div>

    <div class="sc" style="border-left:4px solid var(--yellow)">
      <div class="rw" style="gap:6px"><span style="font-size:14px">${v.emoji}</span> <span style="font-size:12px; font-weight:700; color:var(--text)">${v.label}</span></div>
      <span class="mo" style="font-size:18px; font-weight:800; color:var(--text); margin-top:4px">${w(v.amount)}</span>
      <div class="pm" style="margin-top:6px; background:var(--bg3)"><div class="pf" style="width:${v.pct}%; background:var(--yellow)"></div></div>
      <span class="ti" style="font-size:10px; color:var(--muted2)">${v.pct}% das despesas</span>
    </div>
  </div>
  ...
  </div>`}function Me(e){let t=document.getElementById(`aii`);t&&(t.value=e)}function Ne(){let e=document.getElementById(`mph`);e&&!e.getAttribute(`href`)&&(e.href=`./manifest.json`)}function Pe(){if(!(`serviceWorker`in navigator))return;let e=`
const C='rico-driverflow-v5';
const STATIC=${JSON.stringify([`./`,`./index.html`,`./manifest.json`,`./icons/icon-192.png`,`./icons/icon-512.png`,`./icons/apple-touch-icon.png`,`./icons/favicon.png`])};

self.addEventListener('install', e => {
  self.skipWaiting();
  e.waitUntil(
    caches.open(C).then(c => c.addAll(STATIC).catch(() => {}))
  );
});
...
self.addEventListener('fetch', e => {
  ...
  // Nunca fazer cache de chamadas para IA ou Supabase
