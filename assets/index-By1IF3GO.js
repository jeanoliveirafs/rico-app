...
});`],{type:`application/javascript`});navigator.serviceWorker.register(URL.createObjectURL(e)).then(()=>console.log(`SW registered`)).catch(()=>{})}var H=null;function ke(){window.addEventListener(`beforeinstallprompt`,e=>{e.preventDefault(),H=e,setTimeout(Ae,1500)})}function Ae(){if(window.matchMedia(`(display-mode: standalone)`).matches||document.getElementById(`ibanner`))return;let e=document.createElement(`div`);e.id=`ibanner`,e.style.cssText=`position:fixed;bottom:calc(80px + env(safe-area-inset-bottom,0px));left:50%;transform:translateX(-50%);background:var(--bg2);border:1px solid rgba(74,222,128,.35);border-radius:14px;padding:12px 16px;display:flex;justify-content:space-between;align-items:center;gap:12px;z-index:50;max-width:440px;width:calc(100% - 32px);box-shadow:0 8px 32px rgba(0,0,0,.4)`,e.innerHTML=`<div><p style="font-size:12px;font-weight:700;color:var(--green)">📲 Instalar RICO como App</p><p style="font-size:11px;color:var(--muted2);margin-top:2px">Funciona offline · Dados salvos no dispositivo</p></div><div style="display:flex;gap:8px"><button onclick="document.getElementById('ibanner').remove()" style="background:transparent;border:1px solid var(--border);border-radius:8px;padding:6px 10px;color:var(--muted);font-size:11px">✕</button><button id="btn-install-pwa" style="background:linear-gradient(135deg,#16a34a,#4ade80);border:none;border-radius:8px;padding:7px 14px;color:#06060f;font-size:11px;font-weight:800;cursor:pointer">Instalar</button></div>`,document.body.appendChild(e),document.getElementById(`btn-install-pwa`).onclick=je}async function je(){if(!H)return;H.prompt(),await H.userChoice,H=null;let e=document.getElementById(`ibanner`);e&&e.remove()}function Me(){window.navigator.standalone||/iPhone|iPad|iPod/.test(navigator.userAgent)&&(localStorage.getItem(`ios-hint-shown`)||(localStorage.setItem(`ios-hint-shown`,`1`),setTimeout(()=>{let e=document.createElement(`div`);e.style.cssText=`position:fixed;bottom:calc(90px + env(safe-area-inset-bottom,0px));left:50%;transform:translateX(-50%);background:#1e293b;border:1px solid rgba(74,222,128,.3);border-radius:14px;padding:14px 18px;font-size:12px;color:#e2e8f0;z-index:50;max-width:300px;text-align:center;box-shadow:0 8px 32px rgba(0,0,0,.6)`,e.innerHTML=`📲 <b>Instalar no iPhone:</b><br/>Toque em <b>Compartilhar</b> (⬆️)<br/>depois <b>"Adicionar à Tela Inicial"</b><br/><span style="font-size:10px;color:#475569;margin-top:6px;display:block">Toque para fechar</span>`,e.onclick=()=>e.remove(),document.body.appendChild(e),setTimeout(()=>e.remove(),8e3)},2e3)))}async function Ne(){if(!(`Notification`in window)){alert(`Seu navegador não suporta notificações de área de trabalho.`);return}if(Notification.permission===`granted`){new Notification(`💎 RICO Finanças`,{body:`As notificações já estão ativadas! Avisaremos quando houver contas a vencer.`,icon:`data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 viewBox=%270 0 32 32%27%3E%3Crect width=%2732%22 height=%2232%22 rx=%228%22 fill=%22%2306060f%22/%3E%3Ctext y=%2224%22 x=%2216%22 text-anchor=%22middle%22 font-size=%2220%22%3E💎%3C/text%3E%3C/svg%3E`});return}await Notification.requestPermission()===`granted`?new Notification(`💎 RICO Finanças`,{body:`Notificações ativadas com sucesso! Você receberá alertas de contas a vencer.`,icon:`data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 viewBox=%270 0 32 32%27%3E%3Crect width=%2732%22 height=%2232%22 rx=%228%22 fill=%22%2306060f%22/%3E%3Ctext y=%2224%22 x=%2216%22 text-anchor=%22middle%22 font-size=%2220%22%3E💎%3C/text%3E%3C/svg%3E`}):alert(`Permissão de notificação foi negada no navegador.`)}function U(){if(!(`Notification`in window))return;let e=new Date().getDate(),t=O(),n=c.bills.filter(n=>!n.paid&&n.month===t&&n.dueDay&&n.dueDay<=e+3);if(n.length===0)return;if(!document.getElementById(`bill-notif-bar`)){let e=document.createElement(`div`);e.id=`bill-notif-bar`,e.style.cssText=`position:fixed;top:calc(env(safe-area-inset-top,0px)+56px);left:0;right:0;background:rgba(250,204,21,.95);color:#06060f;padding:8px 16px;font-size:12px;font-weight:700;z-index:100;display:flex;justify-content:space-between;align-items:center;cursor:pointer`;let t=n.slice(0,2).map(e=>e.name).join(`, `)+(n.length>2?` +${n.length-2}`:``);e.innerHTML=`<span>⚠️ ${n.length} conta${n.length>1?`s`:``} pendente${n.length>1?`s`:``}: ${t}</span><button style="background:transparent;border:none;font-size:16px;padding:0 4px;cursor:pointer" id="btn-close-bill-notif">✕</button>`,e.onclick=t=>{t.target.id!==`btn-close-bill-notif`&&(ve(`bills`),e.remove())},document.body.appendChild(e);let r=document.getElementById(`btn-close-bill-notif`);r&&(r.onclick=t=>{t.stopPropagation(),e.remove()}),setTimeout(()=>{let e=document.getElementById(`bill-notif-bar`);e&&e.remove()},8e3)}let r=localStorage.getItem(`rico_last_notif_date`),i=new Date().toISOString().split(`T`)[0];r!==i&&Notification.permission===`granted`&&(n.forEach(e=>{new Notification(`💳 Conta pendente: ${e.name}`,{body:`${T(e.amount)} — vence dia ${e.dueDay}`,icon:`data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 viewBox=%270 0 32 32%27%3E%3Crect width=%2732%22 height=%2232%22 rx=%228%22 fill=%22%2306060f%22/%3E%3Ctext y=%2224%22 x=%2216%22 text-anchor=%22middle%22 font-size=%2220%22%3E💎%3C/text%3E%3C/svg%3E`})}),localStorage.setItem(`rico_last_notif_date`,i))}function Pe(){let e=document.getElementById(`offline-badge`);!l.isOnline&&!e?(e=document.createElement(`div`),e.id=`offline-badge`,e.style.cssText=`position:fixed;bottom:calc(68px + env(safe-area-inset-bottom,0px));right:10px;background:rgba(248,113,113,.9);color:#fff;font-size:10px;font-weight:700;padding:4px 8px;border-radius:10px;z-index:50`,e.textContent=`OFFLINE`,document.body.appendChild(e)):l.isOnline&&e&&e.remove()}function W(e){let t=document.getElementById(`mc`);e===`tx`?t.innerHTML=Fe():e===`bill`?t.innerHTML=Re():e===`debt`?t.innerHTML=Be():e===`shop`?t.innerHTML=He():e===`inv`?t.innerHTML=We():e===`budget`?t.innerHTML=qe():e===`profile`&&(t.innerHTML=Ke()),document.getElementById(`mov`).style.display=`flex`,setTimeout(()=>{let e=document.querySelector(`.sh input`);e&&e.focus()},100)}function G(){document.getElementById(`mov`).style.display=`none`}function Fe(){return`<p class="mt">Nova Transação</p>
  <div class="tt">
...
- Se o usuário possuir dívidas em aberto, lembre-o com prioridade de quitá-las antes de aportar novos investimentos.`;try{let e=await Q(c.aiChat.map(e=>({role:e.role,content:e.content})),h);c.aiChat.push({role:`assistant`,content:e})}catch{c.aiChat.push({role:`assistant`,content:`❌ Erro de conexão. Verifique sua internet.`})}l.al=!1,C(),g()}async function vt(){let e=document.getElementById(`iai`);if(!e||!e.value.trim()||l.ial)return;let t=e.value.trim();e.value=``,l.ial=!0,c.invChat.push({role:`user`,content:t}),g();let n=`${s}
...
Máx 3 parágrafos. Exemplos em R$. Cite corretoras reais. Use princípios sólidos de investimento.`;try{let e=await Q(c.invChat.map(e=>({role:e.role,content:e.content})),n);c.invChat.push({role:`assistant`,content:e})}catch{c.invChat.push({role:`assistant`,content:`❌ Erro.`})}l.ial=!1,C(),g()}async function yt(){let e=c.shoppingList.filter(e=>!e.bought);if(!e.length||l.ap)return;l.ap=!0,g();let t=`${s}
...
`)}
JSON SOMENTE: {"items":[{"id":NUMERO,"priority":"valor","justification":"1 frase"}]}`;try{let e=ht(await Q([{role:`user`,content:t}],`Responda apenas JSON válido sem markdown.`)),n={};(e.items||[]).forEach(e=>{n[e.id]={priority:e.priority,aiJustification:e.justification}}),c.shoppingList=c.shoppingList.map(e=>n[e.id]?{...e,...n[e.id]}:e),C()}catch(e){console.error(`Erro ao priorizar compras:`,e)}l.ap=!1,g()}async function bt(){if(l.ia)return;l.ia=!0,g();let e=c.investments.reduce((e,t)=>e+t.amount,0),t=c.investments.filter(e=>e.type===`reserva`).reduce((e,t)=>e+t.amount,0),{income:n,expense:r,sobra:i,realIncome:a,realExpense:u}=gt(),d=c.debts.filter(e=>!e.paid).reduce((e,t)=>e+t.amount,0),f=`${s}
...
}`;try{l.invA=ht(await Q([{role:`user`,content:f}],"Responda apenas JSON válido. Não inclua marcas markdown ```json ou texto explicativo fora do JSON."))}catch(e){console.error(`Erro no diagnóstico de IA:`,e),l.invA={diagnostico:`Erro ao conectar ou ler dados da IA.`,recomendacoes:[],alertasPrioritarios:[]}}l.ia=!1,C(),g()}function xt(){let e=document.getElementById(`hb`);if(!e)return;e.textContent=T(c.balance),e.style.color=c.balance>=0?`var(--green)`:`var(--red)`;let t=document.getElementById(`huser`);t&&l.currentUser&&(t.textContent=l.currentUser.email?.split(`@`)[0]||``);let n=document.getElementById(`bills-dot`);if(n){let e=c.bills.filter(e=>e.month===O()&&!e.paid);n.style.display=e.length?`block`:`none`}}function $(){let e=document.getElementById(`main`);if(!e)return;e.className=`fade`;let t=l.currentView;t===`dashboard`?e.innerHTML=re():t===`bills`?e.innerHTML=ie():t===`debts`?e.innerHTML=le():t===`shopping`?e.innerHTML=me():t===`invest`?e.innerHTML=xe():t===`ai`?e.innerHTML=Te():t===`history`&&(e.innerHTML=z()),xt(),U(),Pe(),setTimeout(()=>{[`acb`,`icb`].forEach(e=>{let t=document.getElementById(e);t&&(t.scrollTop=t.scrollHeight)})},60)}function St(t){let n=c.transactions.find(e=>e.id===t);n&&q(`Deseja excluir a transação <strong>"${n.desc}"</strong> de <strong>${T(n.amount)}</strong>?`,async()=>{await e.from(`transactions`).delete().eq(`created_id`,String(t)),c.balance-=n.type===`income`?n.amount:-n.amount,c.transactions=c.transactions.filter(e=>e.id!==t),R(),b(),C(),$()})}function Ct(e){l.txFilterMonth=e,l.txPage=0,$()}function wt(e){l.txFilterCat=e,l.txPage=0,$()}function Tt(e){l.txFilterType=e,l.txPage=0,$()}function Et(e){l.txPage=e,$()}function Dt(e){l.txSearch=e,l.txPage=0,$()}function Ot(){l.txFilterMonth=``,l.txFilterCat=``,l.txFilterType=``,l.txSearch=``,l.txPage=0,$()}Object.assign(window,{doLogin:Y,doLogout:lt,doRegister:X,switchAuth:J,toggleTheme:be,sv:ve,om:W,cm:G,render:$,sit:ye,exportCSV:M,exportPDF:N,exportJSON:P,importJSON:F,dtx:St,setTxFilterMonth:Ct,setTxFilterCat:wt,setTxFilterType:Tt,clearTxFilters:Ot,setTxPage:Et,setTxSearch:Dt,etx:$e,utx:et,tbp:ae,db:oe,copyPrevBills:j,ebill:Ye,uebill:Xe,setBillFilter:B,tdp:ue,dd:de,pdp:Ze,appdp:Qe,edebt:tt,udebt:nt,setDebtFilter:ce,tb:he,dsi:ge,eshop:rt,ushop:it,setShopFilter:pe,di:Se,setComp:Ce,sii:we,einv:at,uinv:ot,stt:Ie,stx:Le,sbill:ze,sdebt:Ve,sshop:Ue,sinv:Ge,setBudget:Je,aa:_t,aia:vt,anv:bt,aip:yt,sai:Ee,confirmDelete:q,execDelete:st,requestNotificationPermission:Ne,updateUserPassword:mt});async function kt(){h($);let e=await ee();pt(),ke(),e&&(V(),$())}kt(),new URLSearchParams(window.location.search).get(`action`)===`tx`&&setTimeout(()=>W(`tx`),500);var At=document.getElementById(`btn-login`);At&&At.addEventListener(`click`,Y);var jt=document.getElementById(`btn-register`);jt&&jt.addEventListener(`click`,X);var Mt=document.getElementById(`auth-pass`);Mt&&Mt.addEventListener(`keydown`,function(e){e.key===`Enter`&&Y()});var Nt=document.getElementById(`auth-email`);Nt&&Nt.addEventListener(`keydown`,function(e){e.key===`Enter`&&Y()});var Pt=document.getElementById(`auth-pass-reg`);Pt&&Pt.addEventListener(`keydown`,function(e){e.key===`Enter`&&X()});var Ft=document.getElementById(`auth-email-reg`);Ft&&Ft.addEventListener(`keydown`,function(e){e.key===`Enter`&&X()});